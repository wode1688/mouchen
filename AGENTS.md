...
- `relay/companion.py` 归档收到的文件；`relay/bridge.py` 将配对手机的 `ai-twin.sync/v1` 请求导入隔离的本地业务账户，并回传目标与建议。Android 独立本地处理入口仅接用户主动提交及分享内容，iOS 与旧自动采集仍需分别接入。不要把接口测试描述成手机真机验证。
