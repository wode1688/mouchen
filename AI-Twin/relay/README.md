...
这个目录把轻量云中转服务、电脑端同步组件、配置工具和测试放在同一份源码中。服务端只负责临时存取和传输状态；内容处理在接收设备执行。`companion.py` 提供文件归档；新增 `bridge.py` 可接通 Android 主动提交、电脑本地业务库与规则分析，再将建议回传手机，见[接入说明](../docs/relay-local-sync.md)。
...
本组件测试使用合成数据。Windows 加密记录读取已在开发环境验证；其他系统的文件传输客户端可以运行，但不能解密另一台 Windows 电脑的原应用数据库。Android 的独立本地处理入口使用新的应用消息协议；iOS 尚需接入。接口测试不能当作真实手机端验证。该组件不会运行模型，也不会自动执行收到的文件或建议。
