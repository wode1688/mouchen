...
应用源码在 `AI-Twin/`，共同规则 `AGENTS.md`、`CLAUDE.md` 和 GitHub 配置在仓库根目录。工具打开每个任务的仓库根目录，业务源码和交接记录分别位于其中的 `AI-Twin/` 与 `AI-Twin/docs/handoffs/`；运行应用命令时再进入 `AI-Twin/`。

## 三个工具如何接入

| 工具 | 项目规则入口 | 使用方法 |
|---|---|---|
| Codex | 仓库根目录 `AGENTS.md` | 在对应任务的仓库根目录打开项目，启动新会话并读取当前任务记录 |
| Claude Code | 仓库根目录 `CLAUDE.md` 引用 `AGENTS.md` | 在对应任务的仓库根目录启动 Claude Code，读取同一份规则 |
| Hermes Agent（Nous Research） | 仓库根目录 `AGENTS.md` | 从对应任务的仓库根目录启动 Hermes，读取同一份规则 |
...
```powershell
python AI-Twin/tools/ai_workspace.py --tool codex --task issue-101-sync
python AI-Twin/tools/ai_workspace.py --tool claude --task issue-102-ui
python AI-Twin/tools/ai_workspace.py --tool hermes --task issue-103-review
```
...
```powershell
python AI-Twin/tools/ai_workspace.py --tool codex --task first-task --base origin/codex/cloud-relay-sync
```
...
1. 原执行者更新 `AI-Twin/docs/handoffs/` 中的任务交接文件：目标、完成内容、改动范围、实际检查、未完成事项和下一步。
...
> 在这个任务的仓库根目录继续工作。先读取 AGENTS.md 和 AI-Twin/docs/handoffs/ 中当前任务的记录，核对分支、最近提交和未提交修改。应用源码与运行命令位于 AI-Twin/。按记录完成下一步，保留其他任务的工作；结束时更新交接文件，说明实际测试和剩余事项。若尚未获取 Issue 内容，先说明当前依据的本地记录，不推断远端任务已被认领。
...
本地验证以 `AI-Twin/` 为应用工作目录：虚拟演示执行 `python examples/demo.py`，协作脚本执行 `python -m unittest discover -s tools/tests -v`；中转测试先进入其下的 `relay/`。依赖和受影响平台的完整检查按仓库根目录 [CONTRIBUTING.md](../../CONTRIBUTING.md) 及各组件说明准备。
